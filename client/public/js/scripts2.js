...
function initMap() {
  ...
  console.log("initial searchBox: " + searchBox);
...
function getPlaceResults(searchBox, map){
  ...
  searchBox.addListener('places_changed', getMarkers(searchBox, map))
}

function updatePlaceResults(searchBox, map){
  map.addListener('dragend', getMarkers(searchBox, map))
}

function getMarkers(searchBox, map) {
  var markers = [];
  var newBox = new google.maps.places.SearchBox(document.getElementById('search-term'));
  console.log(newBox);
